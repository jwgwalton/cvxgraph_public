...
def run_simulation(n, A1, multipartite_graph_matrix, iterations, partitions, pertubation_scale = 0.1):
  ...
  for i in range(0,iterations):
    ...
    correct_degree_sequence = check_degree_sequence(p,np.round(A2_star)) # check_degree_sequence can't deal with weighted edges so round

    if correct_degree_sequence:
      adjacency_table = Graph.create_adjacency_table(n,A2_star)
      #print('p=',p,'\ng=',adjacency_table)
      multipartite_graph_recognizer=Multipartite_graph_recognizer(p,adjacency_table)
